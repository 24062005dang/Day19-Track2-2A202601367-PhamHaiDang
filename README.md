...
> giữa `fastembed` (bge-small-en, 384d, tiếng Anh — mặc định lite),
> `fastembed-q` (**cùng** model nhưng bản ONNX int8),
...
>
> `fastembed` vs `fastembed-q` là bài học thứ hai, về **latency**: cùng model,
> cùng 384 chiều, nhưng bản int8 mất ~140 ms/query trên CPU không có AVX-VNNI
> còn bản fp32 chỉ ~12 ms. Vì embed một câu query chiếm gần hết P99 của hybrid,
> riêng chi tiết build đó quyết định NB3 đạt hay trượt ngưỡng 50 ms — xem phần
> phân tích trong NB3.
...
make test            Both: pytest (42 tests, ~20 s)
...
│   ├── 05_filtered_search.py       # nâng cao
│   ├── 06_agent_retrieval.py       # nâng cao
│   ├── 07_semantic_cache.py        # nâng cao
│   └── 08_feature_engineering.py   # nâng cao
├── app/
│   ├── main.py                     # FastAPI /search endpoint
│   ├── search.py                   # Searcher class (kw / sem / hybrid)
│   ├── embeddings.py               # EMBEDDING_BACKEND -> model + dim
│   ├── filters.py                  # NB5: post-filter / pre-filter / filtered-ANN
│   ├── metadata.py                 # NB5: payload schema + selectivity
│   ├── agent.py                    # NB6: retrieval-as-a-tool + planner
│   ├── cache.py                    # NB7: semantic cache (threshold/TTL/namespace)
│   ├── features.py                 # NB8: 6 feature families + encodings
│   ├── feast_repo/
│   │   ├── feature_store.yaml      # Feast config (lite vs docker)
│   │   └── feature_views.py        # 3 feature views definition
│   └── feast_repo_ondemand/
│       └── definitions.py          # NB8: on-demand feature view
├── scripts/
│   ├── seed_corpus.py              # 1000 VN docs + 50 golden queries (deterministic)
│   ├── benchmark.py                # Precision@10 + P99 latency harness
│   ├── gen_agent_queries.py        # NB6 compound queries
│   ├── gen_spend.py                # NB8 spend parquet
│   ├── take_screenshots.py         # renders submission/screenshots/*.png
│   ├── verify_lite.py              # lite smoke test
│   └── verify_docker.py            # docker smoke test
├── tests/                          # 42 pytest tests (~20 s)
